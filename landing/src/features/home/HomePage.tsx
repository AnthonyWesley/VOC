...
const registrationUrl = import.meta.env.VITE_MEMBER_REGISTRATION_URL as string | undefined;
...
export function HomePage() {
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#071126]">
        <p className="text-sm font-medium tracking-[0.2em] uppercase">Carregando...</p>
      </div>
    );
  }
  ...
  const banners =
    data.banners.length > 0
      ? data.banners.map((item) => ({
          ...
          ctaLabel: item.ctaLabel || data.primaryCtaLabel || undefined,
          ctaHref: item.ctaUrl || data.primaryCtaUrl || undefined,
        }))
      : [
          {
            id: 'default',
            ...
            ctaLabel: data.primaryCtaLabel || undefined,
            ctaHref: data.primaryCtaUrl || undefined,
          },
        ];
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-[#071126] text-white">
      ...
      <div
        ref={topbarRef}
        className="absolute top-0 left-0 z-40 w-full border-b border-white/10"
      >
        <div className="container mx-auto flex min-h-16 flex-col gap-3 px-4 py-3 sm:px-6 lg:h-16 lg:flex-row lg:items-center lg:justify-between lg:px-16">
          ...
          <div className="flex items-center justify-center">
            {data.facebookUrl && (
              <TopIcon icon="mdi:facebook" href={data.facebookUrl} label="Facebook" />
            )}

            {data.instagramUrl && (
              <TopIcon icon="mdi:instagram" href={data.instagramUrl} label="Instagram" />
            )}

            {data.youtubeUrl && (
              <TopIcon icon="mdi:youtube" href={data.youtubeUrl} label="YouTube" />
            )}

            {data.contactEmail && (
              <TopIcon icon="mdi:email-outline" href={`mailto:${data.contactEmail}`} label="E-mail" />
            )}
          </div>
        </div>
      </div>
      ...
      <header
        className={`fixed left-0 z-50 w-full transition-all duration-300 ${
          scrolled ? 'top-0 bg-slate-950/95 shadow-lg backdrop-blur' : 'bg-transparent'
        }`}
        style={!scrolled ? { top: topbarHeight || 64 } : undefined}
      >
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-16">
          <div className="select-none">
            <img src="/images/logo-white.png" alt={data.churchName} className="h-5 w-auto sm:h-6" />
          </div>

          <nav className="hidden items-center gap-6 lg:flex">
            <NavItem label="Home" target="home" />
            <NavItem label="Ministérios" target="ministerios" />
            {registrationUrl && <CadastroLink href={registrationUrl} />}
        ...
        {mobileMenuOpen && (
          <div className="border-t border-white/10 bg-[#071126] lg:hidden">
            <div className="flex flex-col px-6 py-4">
              <MobileNavItem label="Home" target="home" onClick={() => setMobileMenuOpen(false)} />
              <MobileNavItem label="Ministérios" target="ministerios" onClick={() => setMobileMenuOpen(false)} />
              ...
              <MobileNavItem label="Mídia" target="midia" onClick={() => setMobileMenuOpen(false)} />
              <MobileNavItem
                label="Comunicação"
                target="comunicacao"
                onClick={() => setMobileMenuOpen(false)}
              />
              <MobileNavItem label="Sobre Nós" target="sobre" onClick={() => setMobileMenuOpen(false)} />
              <MobileNavItem label="Contato" target="contato" onClick={() => setMobileMenuOpen(false)} />
            </div>
          </div>
        )}
      </header>
      ...
      {/* INSTAGRAM / FOTOS */}
      <InstagramGallery fallbackItems={data.photos} />

      {/* MIDIA */}
      <section id="midia" className="scroll-mt-32 bg-[#0b1320] px-4 py-24 sm:px-6">
      ...
      >
        <div className="mb-14 text-center">
          ...
          <h2 className="mt-5 text-3xl font-black uppercase sm:text-4xl lg:text-5xl">Notícias</h2>
        </div>
        ...
        <div className="max-w-3xl text-center">
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-4">
            {data.secondaryCtaLabel && data.secondaryCtaUrl && (
              <ActionButton label={data.secondaryCtaLabel} href={data.secondaryCtaUrl} secondary />
            )}
          </div>
        </div>
      </section>
      ...
      {/* CONTATO */}
      <section id="contato" className="flex scroll-mt-32 items-center justify-center px-4 py-24 sm:px-6">
        <div className="mx-auto max-w-xl text-center">
          ...
          <div className="mt-10 space-y-8 text-slate-300">
            <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              <Icon icon="mdi:map-marker-outline" className="text-2xl text-sky-700" />
            ...
            <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              <Icon icon="mdi:phone-outline" className="text-2xl text-sky-700" />

              <p>{data.contactPhone}</p>
            </div>

            <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              <Icon icon="mdi:email-outline" className="text-2xl text-sky-700" />
      ...
      <footer className="border-t border-white/10 bg-[#050b16]">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-6 px-4 py-10 text-center sm:px-6 lg:flex-row lg:px-6 lg:text-left">
          ...
          <div className="flex items-center gap-4">
            {data.facebookUrl && (
              <FooterIcon icon="mdi:facebook" href={data.facebookUrl} label="Facebook" />
            )}

            {data.instagramUrl && (
              <FooterIcon icon="mdi:instagram" href={data.instagramUrl} label="Instagram" />
            )}

            {data.youtubeUrl && (
              <FooterIcon icon="mdi:youtube" href={data.youtubeUrl} label="YouTube" />
            )}
          </div>
        </div>
      </footer>
...
function CadastroLink({ href }: { href: string }) {
  return (
    <a
      href={href}
      target="_blank"
...
function ActionButton({
  ...
  const className = secondary ? 'border border-white/10 bg-transparent hover:bg-white/5' : 'bg-sky-700 hover:bg-[#172554]';
