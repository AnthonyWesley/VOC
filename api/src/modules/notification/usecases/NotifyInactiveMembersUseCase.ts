...
export class NotifyInactiveMembersUseCase {
  ...
  async execute(): Promise<InactiveMembersProcessingSummary> {
    ...
    const recentMemberIdsByAdmin = new Map<string, Set<string>>();
    for (const admin of adminUsers) {
      const recent = await this.prisma.notification.findMany({
        where: { userId: admin.id, type: "MEMBER_AUSENTE", createdAt: { gte: thirtyDaysAgo } },
        select: { payload: true },
      });
      const memberIds: string[] = recent
        .map((n) => (n.payload as { memberId?: string } | null)?.memberId)
        .filter((id): id is string => Boolean(id));
      recentMemberIdsByAdmin.set(admin.id, new Set(memberIds));
    }
    ...
    for (const type of eventTypes) {
      ...
      for (const member of inactiveMembers) {
        ...
        for (const admin of adminUsers) {
          const recentCount = recentMemberIdsByAdmin.get(admin.id)?.has(member.id) ? 1 : 0;
