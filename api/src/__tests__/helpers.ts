...
export async function seedTestUser(prisma: PrismaClient, roleLevel: number = 80) {
  ...
  const extraUsers = [
    { id: "u-1", email: "u1@t.com" },
    { id: "u-2", email: "u2@t.com" },
    { id: "u-r", email: "ur@t.com" },
    { id: "user-canceller", email: "uc@t.com" },
    { id: "u-conc", email: "uconc@t.com" },
  ];
  for (const u of extraUsers) {
    await prisma.user.upsert({
      where: { id: u.id },
      update: { email: u.email },
      create: { id: u.id, email: u.email, passwordHash: "h", isActive: true },
    });
  }
